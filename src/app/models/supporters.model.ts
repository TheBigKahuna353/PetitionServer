...
const getNumSupporters = async (id: number): Promise<string> => {
...
const getMoneyRaised = async (id: number): Promise<string> => {
    Logger.info(`Getting number of supporters from the database for petition ${id}`);
    const conn = await getPool().getConnection();
    const query = 'SELECT COUNT(supporter.id) AS number_of_supporters, ' +
                        'SUM(cost) AS money_raised, ' +
    ...
    return rows[0].money_raised;
}

const getNumSupportersFromTier = async (id: number): Promise<string> => {
    Logger.info(`Getting number of supporters from the database for petition ${id}`);
    const conn = await getPool().getConnection();
    const query = 'SELECT COUNT(supporter.id) AS number_of_supporters ' +
                        'FROM supporter JOIN support_tier ON supporter.support_tier_id=support_tier.id ' +
...
export {getNumSupporters, getNumSupportersFromTier, getAllSupportersForPetition, getAllSupportersForTier, insertSupporter, getMoneyRaised};